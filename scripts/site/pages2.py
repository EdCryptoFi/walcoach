...
<main class="relative z-10 pt-28 max-w-[1440px] mx-auto px-4 lg:px-8">
  ...
  <section class="grid md:grid-cols-2 xl:grid-cols-4 gap-5 mt-10">
    ...
    {step("03","lock","3. Blobs on Walrus","Each fact is sent to the Walrus Memory relayer, which encrypts it (SEAL) and stores it as a blob on Walrus mainnet, in the account your key owns on Sui.","enhanced_encryption","Encrypted at rest, owned on-chain")}
  ...
  <section class="glass rounded-[2rem] p-6 lg:p-8 mt-12">
    ...
    <div class="grid md:grid-cols-3 gap-4 mt-6">
      ...
      <div class="glass-hi rounded-2xl p-5"><span class="chip">Relayer · SEAL encryption</span><h3 class="font-semibold mt-3">Encrypted packaging</h3><p class="text-sm text-on-surface-variant mt-2">The Walrus Memory relayer embeds each fact for search, encrypts it with SEAL (threshold encryption tied to your account) and uploads it. The relayer processes the text in clear; our servers keep nothing.</p><div class="mt-3 text-xs font-mono bg-white rounded-xl p-3 border border-white">{icon('key','text-[14px] align-middle')} account: owned by your key</div></div>
  ...
  <section class="mt-16">
    ...
    <div class="grid lg:grid-cols-2 gap-5 mt-8">
      ...
      <div class="glass-hi rounded-3xl p-6 border-primary-container/30">
        ...
        <ul class="mt-4 space-y-3 text-sm">
          <li class="flex gap-3">{icon('check_circle','text-primary text-[20px]')}<div><b>No login, no password</b><div class="text-on-surface-variant">A memory key generated in your browser is your identity, and it owns your account. No wallet required.</div></div></li>
          <li class="flex gap-3">{icon('check_circle','text-primary text-[20px]')}<div><b>Encrypted blobs, owned on-chain</b><div class="text-on-surface-variant">Facts are SEAL-encrypted by the Walrus Memory relayer and stored on Walrus, in an account that belongs to you on Sui.</div></div></li>
        ...
        <div class="flex justify-between text-xs mt-5 pt-4 border-t border-white/60"><span class="text-on-surface-variant">Data custody</span><span class="font-semibold text-primary">Your account, your key</span></div>
      </div>
    </div>
  </section>
...
<main class="relative z-10 pt-28 max-w-[1440px] mx-auto px-4 lg:px-8">
  ...
  <section class="glass-hi rounded-[2rem] p-6 lg:p-10 mt-6 grid lg:grid-cols-[1.2fr_.8fr] gap-8 items-center">
    <div>
      ...
      <p class="text-on-surface-variant mt-4 max-w-[60ch]">WalCoach never sells your data, builds no ad profile and keeps no database of your conversations. Every fact the coach learns is encrypted and stored on the Walrus network, in an account that your memory key owns. Below is exactly how, including the parts we don't control.</p>
  ...
  <section class="mt-16">
    ...
    <div class="grid lg:grid-cols-2 gap-5 mt-8">
      {pillar("01","lock","Encryption at rest on Walrus","Each fact is encrypted with SEAL by the Walrus Memory relayer before it is written to Walrus, and can only be decrypted for your account. Blobs on the network are unreadable to storage nodes.", p1)}
      {pillar("02","hub","Decentralised storage on Walrus","Built on Walrus Memory (Mysten Labs): blobs are erasure-coded across storage nodes, and your account is an object on Sui mainnet that lists you as its owner. No single server of ours holds your history, we have no database at all.", p2)}
  ...
  <section class="glass rounded-[2rem] p-6 lg:p-8 mt-16">
    ...
    <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-8 text-center text-sm">
      ...
      <div><span class="w-12 h-12 rounded-full bg-white grid place-items-center text-primary mx-auto">{icon('scatter_plot')}</span><div class="text-xs text-on-surface-variant mt-3">Step 4</div><b>Blob on Walrus mainnet</b><p class="text-on-surface-variant mt-1">Spread across nodes, in your Sui account, linked from the chat.</p></div>
    </div>
  </section>
  ...
  <section class="mt-16">
    ...
    <div class="grid md:grid-cols-3 gap-5 mt-8">
      <article class="glass rounded-3xl p-6"><span class="w-10 h-10 rounded-xl bg-error-container grid place-items-center text-on-error-container">{icon('warning')}</span><h3 class="font-semibold mt-4">What if I lose my key?</h3><p class="text-sm text-on-surface-variant mt-2">There is no password reset. The key is what owns your account, and nobody, including us, can recreate it. Start again with a new key and a new account.</p><ul class="text-xs mt-3 space-y-1"><li class="flex gap-2">{icon('check_small','text-[16px] text-primary')} Keep it in a password manager or notes app</li><li class="flex gap-2">{icon('check_small','text-[16px] text-primary')} Or in <a class="underline" href="https://walnotes.xyz" target="_blank" rel="noopener">Walnotes</a> (encrypted notes on Walrus)</li></ul></article>
  ...
  <section class="mt-16 max-w-3xl mx-auto">
    ...
    <div class="mt-6 space-y-3">
      ...
      <details class="glass rounded-2xl p-5 group"><summary class="cursor-pointer font-semibold flex items-center justify-between">{icon('payments','text-primary')}<span class="flex-1 ml-3">Do I pay gas or need a wallet?</span>{icon('expand_more','group-open:rotate-180 transition')}</summary><p class="text-sm text-on-surface-variant mt-3">No tokens and no wallet. Your browser does sign the transaction that creates your account, with the key it generated, but we pay the gas for it, so there is nothing to install, fund or approve.</p></details>
