...
<main class="relative z-10 pt-28 max-w-3xl mx-auto px-4 lg:px-8">
  ...
  <section class="glass rounded-3xl p-6 mt-4"><h2 class="font-display text-xl font-semibold">No sign-up, no cookies</h2><p class="mt-2 text-sm leading-relaxed">There is no sign-up and no login. On your first visit the browser generates a <strong>memory key</strong> (<code>wal_…</code>), which is a private key, and stores it in <em>localStorage</em> on your device together with the name you typed. That key opens a <strong>Walrus Memory account owned by you</strong> on the Sui network, and your facts are written there. The key itself <strong>never leaves your browser</strong>: it signs the account setup locally, and from then on your requests carry only the account id, which is public. We set no cookies and run no analytics or ad trackers.</p><p class="mt-2 text-sm leading-relaxed">We pay the network fee for creating your account, so you need no wallet and no tokens. WalCoach is added to your account as a <strong>guest key</strong> so it can read and write your memories, and you can remove it, which locks the coach out for good.</p><p class="mt-2 text-sm leading-relaxed">Anyone who has your memory key can talk to the coach as you and read your memories. Keep it private, like a password, somewhere you'll find it again (a notes app, paper, or an encrypted note on <a class="text-secondary font-semibold underline" href="https://walnotes.xyz" target="_blank" rel="noopener noreferrer">Walnotes</a>). There is no recovery if you lose it.</p></section>
  ...
  <section class="glass rounded-3xl p-6 mt-4"><h2 class="font-display text-xl font-semibold">Where your data goes</h2>
    <div class="overflow-x-auto mt-3"><table class="w-full text-sm"><thead><tr class="text-left text-[11px] uppercase tracking-wider text-on-surface-variant"><th class="pb-2 pr-4">Data</th><th class="pb-2 pr-4">Goes to</th><th class="pb-2">Why</th></tr></thead><tbody>
      ...
      {row("Extracted facts about you (e.g. “Ana runs on Tuesdays”)","Walrus Memory relayer → Walrus network","The relayer embeds and encrypts each fact (SEAL) and stores it as a blob on Walrus, a decentralised storage network, in the account your memory key owns. The relayer sees the text in clear while processing it.")}
    ...
    <p class="text-xs text-on-surface-variant mt-3">We keep no database of users. Everything the coach knows about you lives in your own Walrus Memory account and in your browser.</p></section>

  <section class="glass rounded-3xl p-6 mt-4"><h2 class="font-display text-xl font-semibold">Deleting your data</h2><p class="mt-2 text-sm leading-relaxed">To cut WalCoach off from your memories, remove its guest key from your account: it can then no longer read or write anything of yours, and the account stays yours. Clearing the site's data in your browser forgets the memory key, and "Switch user" starts over with a fresh account. Memories already stored on Walrus persist for the storage period of the blob and cannot currently be deleted one by one; they are encrypted and cannot be read without your account. If you need something removed, open an issue on the project repository.</p></section>
