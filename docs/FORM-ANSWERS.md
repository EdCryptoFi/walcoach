...
| Explorer link to the MemWalAccount object | https://suiscan.xyz/mainnet/object/0xf059b5c5b2429903359263c84c78190f6ac70063182d3b44eccb91e31bff7f3d<br>A per-user account: https://suiscan.xyz/mainnet/object/0x9602dff07a0953e6ffce2f435b74643f6deff6fcd65c981dcbabb772ae5320fe |
...
> `MemWal.create({ key, accountId })` accepts an account id and signs it into every request, but the relayer resolves the account from the **delegate key** (on-chain lookup in `MemWalAccount.delegate_keys`) and ignores the id. One delegate key registered on four different accounts returns an identical result set for all four, and so does an account id that **does not exist at all** (`0xabab…abab`), which is the clearest sign the field neither routes nor authorises. I hit this the day I gave every user their own account: each owner was reading and writing one shared pool while my UI told them their memories were in their own account. Caught within hours, only my own test accounts existed, but in a larger deployment this is a cross-user data leak. My fix was one delegate key per account, derived with `HMAC-SHA256(seed, accountId)` so nothing has to be stored. My request: return 400 when the signed account id is not the account the key resolves to, or when it does not exist, and say in the SDK types that the key selects the account. Environment: `@mysten-incubation/memwal` 0.1.7, Node 22 on Vercel functions, relayer.memory.walrus.xyz (mainnet). Full measurements: `docs/RELAYER-ACCOUNT-SCOPING.md`.
...
> ✏️ Open the issues from [FRICTION.md](https://github.com/EdCryptoFi/walcoach/blob/main/FRICTION.md) at https://github.com/MystenLabs/MemWal/issues, then list them here as "short description, link". The nine documented points, the account-id one first since it is the only one that can leak data across users: transient 500/503 on `/api/analyze`; `seal encrypt failed: Too Many Requests` from the relayer's own Sui RPC; prompt template leaking into an extracted fact; `analyze` flipping output language for the same user; no relevance threshold and undocumented cross-language distances; same `job_id` returned on a retried `rememberAndWait`; empty recall with HTTP 200; rate limits per delegate key undocumented; `accountId` accepted and signed but ignored in favour of the delegate key.
